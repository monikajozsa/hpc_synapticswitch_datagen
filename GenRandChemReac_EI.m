...
for j=1:size(connections,1)
    i=connections(j);
    if W(i)==1%excitatory
        Knum_interval=rate_const_intervals.Knum_E;
        Kdenum_interval=rate_const_intervals.Kdenum_E;
        Knum_E(i) = Knum_interval(1)+rand(1)*diff(Knum_interval);
...
%% Added later as a simplification
rate_constants.Kdenum_E=rate_constants.Knum_E;
rate_constants.Kdenum_I=rate_constants.Knum_I;

%% death rates - determined by the rate_constants 
beta=zeros(1,Nspecies);
for i=1:Nspecies
    beta(i)=sum([Knum_E(:,i);Knum_I(:,i)]./[Kdenum_E(:,i)+1;Kdenum_I(:,i)+1]);
